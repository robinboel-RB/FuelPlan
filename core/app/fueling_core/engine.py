"""Pure Python fueling engine for FuelPlan calculations.

The module keeps each sport-science formula as a named function so the
...
def calculate_hr_adjusted(point: SessionPoint) -> float:
    """Removes heat-driven HR inflation above 24 C; output unit: bpm."""
...
def calculate_speed_m_per_min(point: SessionPoint) -> float:
    """Converts pace to running speed; output unit: m/min."""
...
def calculate_fuel_factor(rer_dynamic: float, rer_base: float) -> float:
    """Adjusts Keytel for current substrate mix; output unit: factor."""
...
def calculate_kcal_per_min(dominant_energy_kj_min: float) -> float:
    """Converts dominant kJ to kcal; output unit: kcal/min."""
...
def calculate_fueling_plan(
    ...
    """Runs the minute chain and carb triggers; output unit: dict."""
    ...
    for index, point in enumerate(session_points):
        ...
        # Detect when cumulative carbohydrate burn crosses the next trigger size.
...
def validate_user_inputs(inputs: UserStaticInputs) -> UserStaticInputs:
    """Validates static athlete inputs; output unit: UserStaticInputs."""
...
def validate_session_point(point: SessionPoint) -> None:
    """Validates watch/session inputs; output unit: None."""
